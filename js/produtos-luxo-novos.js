...
window.PRODUTOS.push({id:1204,nome:'Camisa Barcelona Versão Jogador 2026/27 Visitante',categoria:'Camisas',marca:'Catálogo GLT',preco:199.99,tipo:'La Liga • Barcelona',tamanhos:['P','M','G','GG','XGG'],cores:['Conforme imagens'],descricao:'Camisa do Barcelona versão jogador 2026/27 visitante, selecionada no catálogo Wanfing.',img:'assets/images/camisas/europa/la-liga/barcelona/250446952/foto-01.png',galeria:['assets/images/camisas/europa/la-liga/barcelona/250446952/foto-01.png','assets/images/camisas/europa/la-liga/barcelona/250446952/foto-02.png'],clube:'Barcelona',liga:'La Liga',caracteristicas:['Versão jogador','Camisa visitante','Disponibilidade sob consulta']});
...
{
  const p = window.PRODUTOS.find(x => x.id === 1221);
  if (p && p.galeria.length >= 17) {
    const base = p.galeria[0].replace(/foto-\d+\.[^/]+$/, '');
    const nomes = ['foto-01.jpg','foto-02.jpg','foto-03.jpg','foto-04.jpg','foto-11.jpg','foto-12.jpg','foto-14.jpg','foto-15.jpg','foto-16.jpg'];
    p.galeria = nomes.map(n => `${base}${n}`);
    p.img = p.galeria[0];
  }
}
{
  const p = window.PRODUTOS.find(x => x.id === 1222);
  if (p && p.galeria.length >= 20) {
    const base = p.galeria[0].replace(/foto-\d+\.[^/]+$/, '');
    const nomes = ['foto-01.jpg','foto-03.jpg','foto-08.jpg','foto-11.jpg','foto-12.jpg','foto-13.jpg','foto-15.jpg','foto-16.jpg','foto-17.jpg','foto-18.jpg','foto-19.jpg','foto-20.jpg'];
    p.galeria = nomes.map(n => `${base}${n}`);
    p.img = p.galeria[0];
  }
}
{
  const p = window.PRODUTOS.find(x => x.id === 1223);
  if (p && p.galeria.length >= 14) {
    const base = p.galeria[0].replace(/foto-\d+\.[^/]+$/, '');
    const nomes = ['foto-01.png','foto-03.png','foto-10.png','foto-11.png','foto-13.png','foto-14.png'];
    p.galeria = nomes.map(n => `${base}${n}`);
    p.img = p.galeria[0];
  }
}
...
{
  const p = window.PRODUTOS.find(x => x.id === 1225);
  if (p && p.galeria.length >= 14) {
    const base = p.galeria[0].replace(/foto-\d+\.[^/]+$/, '');
    const nomes = ['foto-01.png','foto-02.png','foto-06.png','foto-08.png','foto-12.png','foto-14.png'];
    p.galeria = nomes.map(n => `${base}${n}`);
    p.img = p.galeria[0];
  }
}
{
  const p = window.PRODUTOS.find(x => x.id === 1227);
  if (p && p.galeria.length >= 22) {
    const base = p.galeria[0].replace(/foto-\d+\.[^/]+$/, '');
    const nomes = ['foto-01.jpg','foto-09.jpg','foto-10.jpg','foto-11.jpeg','foto-12.jpg','foto-16.jpg','foto-17.jpg','foto-18.jpg','foto-19.jpg','foto-20.jpg','foto-21.jpg','foto-22.jpeg'];
    p.galeria = nomes.map(n => `${base}${n}`);
    p.img = p.galeria[0];
  }
}
...
window.PRODUTOS.push({
  ...
  categoria:'Camisas', marca:'Catálogo GLT', preco:179.99,
